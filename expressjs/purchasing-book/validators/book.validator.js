...
exports.bookPurchaseValidator = async (req, res, next) => {
  ...
  if (reqBody?.additional_price) {
    if (!reqBody?.additional_price.hasOwnProperty('price')) {
      errors.push('additional_price must an object and has price property');
    }
    if (!reqBody?.additional_price.hasOwnProperty('target_term')) {
...
exports.bookCreateValidator = async (req, res, next) => {
  const reqBody = req?.body;
  const errors = [];
...
exports.bookUpdateValidator = async (req, res, next) => {
  const reqBody = req?.body;
  const errors = [];
